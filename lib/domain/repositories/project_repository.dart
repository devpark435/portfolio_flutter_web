...
''',
      ...
      Project(
        ...
        background: '''
Flutter 생태계에 고기능의 인터랙티브 마인드맵 위젯이 부족하다는 점에 착안하여 개발을 시작했습니다. 
복잡한 데이터 구조를 시각적으로 명확하게 표현하고, 사용자와의 동적인 상호작용을 통해 
정보 탐색의 즐거움을 제공하는 것을 목표로 했습니다.
''',
        meaning: '''
CustomPainter와 복잡한 수학적 계산을 통해 직접 렌더링 파이프라인과 레이아웃 알고리즘을 구현하며 
Flutter 렌더링 시스템에 대한 깊은 이해를 얻을 수 있었습니다. 위젯 API를 설계하고 pub.dev에 
배포하며 오픈소스 생태계에 기여하는 경험을 쌓았습니다. 
전 세계 개발자들의 피드백을 통해 프로젝트를 개선하며 글로벌 협업 역량을 키웠습니다.
''',
        description:
            '다양한 레이아웃, 스마트 카메라, 높은 상호작용성을 자랑하는 Flutter 마인드맵 위젯 라이브러리입니다.',
        technologies: [
          'Flutter',
          'Dart',
          'CustomPainter',
        ...
        challenges: [
          '다양한 레이아웃 알고리즘 구현 및 최적화',
          '대규모 노드(1000개 이상) 환경에서의 렌더링 성능 확보',
          '직관적이고 유연한 위젯 API 설계',
          '다양한 플랫폼(모바일, 웹, 데스크톱)에서의 일관된 동작 보장',
        ],
        responsibilities: [
        ...
        troubleshooting: [
          Troubleshooting(
            issue: '동적 노드 추가/삭제 시 노드 겹침 현상',
            context:
                '마인드맵에 동적으로 노드를 추가하거나 삭제할 때, 레이아웃 알고리즘이 실시간으로 위치를 재계산하지 못해 노드들이 서로 겹쳐 보이는 문제 발생',
            solution:
                '노드 변경(추가/삭제/크기 변경) 시 레이아웃을 다시 계산하고 위젯을 재구성하는 반응형 로직 도입. 변경된 노드와 그 이웃 노드들만 부분적으로 업데이트하는 최적화 알고리즘을 적용하여 전체 맵을 다시 그리는 비용을 최소화',
            learning:
                '복잡한 시각적 데이터 구조에서 실시간 상호작용을 처리하기 위한 효율적인 상태 관리 및 부분 렌더링 최적화 기법의 중요성을 이해함',
          ),
          Troubleshooting(
            issue: '사용자 정의 위젯으로 노드 커스텀 시 오버플로우 발생',
            context:
                '사용자가 노드의 내용물로 텍스트 길이가 가변적인 커스텀 위젯을 제공했을 때, 노드의 크기가 내용에 맞게 동적으로 조절되지 않아 렌더링 오버플로우 에러 발생',
            solution:
                '커스텀 위젯의 크기를 내부적으로 측정한 후, 측정된 크기에 맞춰 부모 노드의 크기를 동적으로 재조정하는 로직 추가. LayoutBuilder를 활용하여 자식 위젯의 실제 크기를 파악하고 이에 맞춰 레이아웃을 다시 계산',
            learning:
                'Flutter의 레이아웃 시스템에 대한 깊은 이해와, 부모-자식 위젯 간의 크기 제약 조건(constraints)을 유연하게 처리하는 방법 습득',
          ),
          Troubleshooting(
            issue: '특정 노드 선택 시 부드러운 카메라 포커싱 구현',
            context:
                '단순히 노드 위치로 카메라를 즉시 이동시키면 사용자가 시각적 맥락을 잃기 쉬웠고, 애니메이션이 없을 경우 경험이 딱딱하게 느껴짐',
            solution:
                '카메라의 현재 위치와 목표 노드의 위치를 보간(interpolate)하는 애니메이션 컨트롤러 구현. Tween과 Curve를 사용하여 가속/감속 효과를 적용, 사용자가 카메라의 이동 경로를 자연스럽게 따라갈 수 있도록 하여 시각적 연속성을 보장함',
            learning:
                '사용자 경험(UX)에서 시각적 연속성의 중요성을 깨닫고, 이를 구현하기 위한 Flutter의 애니메이션 시스템 활용 능력 심화',
          ),
        ],
        improvements: [
          '더 많은 노드 커스텀 옵션 추가',
          '키보드 단축키 지원',
          '마인드맵 데이터 가져오기/내보내기 기능',
        ],
